import os
os.environ['GRADIO_TEMP_DIR'] = '/home/slyin/theta/Era3D/tmp'
...
scene = 'scene'
@spaces.GPU
def run_pipeline(pipeline, cfg, single_image, guidance_scale=3, steps=40, seed=600, crop_size=420, chk_group=None):
...
def run_demo():
    ...
    with gr.Blocks(title=_TITLE, theme=custom_theme, css=custom_css) as demo:
        ...
        print('Launching...')
        run_btn.click(
            fn=partial(preprocess, predictor), inputs=[input_image], outputs=[processed_image_highres, processed_image], queue=True
        ).success(
            fn=partial(run_pipeline, pipeline, cfg),
            inputs=[processed_image_highres],
            outputs=[view_gallery, normal_gallery],
        )
        recon_btn.click(
            process_3d, inputs=[mode, data_dir], outputs=[obj_3d]
        )
